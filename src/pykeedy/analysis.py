...
def frequency_rank(text: str | list[str], n: int = 1, normalize: bool = True) -> dict[str, float]:
...
def cooccurence_matrix(text: str | list[str], n: int = 2, normalize: bool = True) -> tuple[list[str], list[list[float]]]:
    char_level = isinstance(text, str)
    seq = np.array(list(text)) if char_level else np.array(text)
...
def all_pos(text: str, substring: str, word_mode: bool = True, normalize: bool = True) -> list[float | int]:
    ...
    while True:
        ...
        if word_mode: # Word mode - check boundaries
            ...
            end_valid = index + word_len == len(text) or text[index + word_len] in word_bounds
...
def position_distribution(items: list[str] | set[str], sequences: list[str], word_mode: bool = False, normalize: bool = True, average: bool = False) -> dict[str, list[int | float] | int | float]:
    """
    ...
    if average:
        ...
        ordered = {k: results[k] for k in items if k in results}  # preserve original order
        results = ordered
    else:
