...
def analysis_to_md(
    ...
    for pattern in cross_manuscript:
        ...
        markdown_lines.append(f'<p float="left">')
    ...
    individual_width = total_width // num_manuscripts if num_manuscripts > 0 else total_width
    ...
    for analysis_type, pattern_list in per_manuscript.items():
        for pattern in pattern_list:
            ...
            for manuscript in manuscripts:
                ...
                markdown_lines.append(f'  <img src="{image_src}" width="{individual_width}" />')
