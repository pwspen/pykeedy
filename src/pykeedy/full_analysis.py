...
from pykeedy.analysis import shannon_entropy, conditional_entropy, frequency_rank, cooccurence_matrix, length_distribution, position_distribution
...
def cross_manuscript(
    ...
    if run_entropy:
        ...
        scatterplot(entropy, key=(f"character entropy (bits)", "conditional entropy (bits)"), fname=f"{output_dir}/entropy_comparison.png")
...
def per_manuscript(
    ...
    barplot_colors = ['red', 'blue', 'green', 'orange', 'purple', 'brown', 'pink', 'gray', 'olive', 'cyan']
    ...
    for mode in ["char", "word"]:
        if run_ngrams:
            ...
            for i in range(ngram_max_n):
                gramsize = i + 1
                barplot(frequency_rank(text, n=gramsize), key=(f"{mode} {gramsize}-gram", "Frequency"), fname=f"{output_dir}/{name}_{mode}_{gramsize}-gram_freq.png", color=color, title=f"{name} ngram freq")
        if run_heatmaps:
            # Generate heatmaps
            res = cooccurence_matrix(text, n=2)
            heatmap(*res, fname=f"{output_dir}/{name}_{mode}_cooccurence_heatmap.png", title=f"{name} {mode} co-occurence")
    ...
    if run_positions:
        ...
        barplot(many_occurs, key=("Letter", "Avg pos in word"), fname=f"{output_dir}/{name}_letters_pos_in_words.png", n_max=len(many_occurs), color=color, title=f"{name} letter positions in words")

        # Positions of 20 most common words
        top_words = [k for k, v in frequency_rank(word_tokens).items()][:20]

        barplot(position_distribution(top_words, lines, word_mode=True, average=True), key=("Word (most common on left)", "Avg pos in line"), fname=f"{output_dir}/{name}_words_pos_in_lines.png", color=color, title=f"{name} word positions in lines")
        barplot(position_distribution(top_words, [text], word_mode=True, average=True), key=("Word (most common on left)", "Avg pos in VMS"), fname=f"{output_dir}/{name}_words_pos_in_manuscript.png", color=color, title=f"{name} word positions in manuscript")
...
def run_full_analysis(
    ...
    run_per_manuscript = any([run_ngram_freqs, run_cooccurence, run_word_lengths, run_positions])
    ...
    analysis_manifest = {
        "analysis": {
            ...
            "per_manuscript": {
                ...
                "char_ngram_freq": ["<name>_char_1-gram_freq.png", "<name>_char_2-gram_freq.png", "<name>_char_3-gram_freq.png"],
                "word_ngram_freq": ["<name>_word_1-gram_freq.png", "<name>_word_2-gram_freq.png", "<name>_word_3-gram_freq.png"],
                "cooccurence_heatmaps": ["<name>_char_cooccurence_heatmap.png", "<name>_word_cooccurence_heatmap.png"],
                "word_lengths": ["<name>_word_lengths.png"],
                "positions": ["<name>_letters_pos_in_words.png", "<name>_words_pos_in_lines.png", "<name>_words_pos_in_manuscript.png"],
            }
        }
    }
    ...
    finish = time.time()
    print(f"Completed analysis of {len(texts)} manuscripts in {finish - start:.2f} seconds")