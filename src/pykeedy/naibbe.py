...
class NaibbeEncoding(BaseModel):
    ...
    def from_name(cls, name: str) -> "NaibbeEncoding":
        ...
        if name not in encodings:
            raise ValueError(f"Encoding '{name}' not found. Available encodings: {list(encodings.keys())}")
    ...
    def from_file(cls, filepath: Path) -> "NaibbeEncoding":
        ...
        try:
            ...
            raise ValueError("Encoding YAML file must contain 'encoding' as top-level key")
        ...
        if len(tables) == 0:
            raise ValueError(f"No valid encoding tables found in file. Valid names: {valid_tables}")
    ...
    def to_file(self, filepath: Path) -> None:
        ...
        valid_tables = ("unigram", "bigram_prefix", "bigram_suffix", "trigram_prefix", "trigram_core", "trigram_suffix")
