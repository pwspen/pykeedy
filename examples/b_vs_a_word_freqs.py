...
for lang in ("A", "B"):
    ...
    text = VMS.filter(
        [LocusProp.CurrierLanguage(lang)]
    ).to_words()  # Get words list (for char level: .to_text())
    barplot(
        frequency_rank(text), fname=f"vms_{lang}_word_freq.png"
    )  # Normalizes by default, normalize=False to change
